...
export default function DerivAnalysis() {
  ...
  useEffect(() => {
    ...
    derivAPI.onOpen(() => {
      ...
      derivAPI
        ...
        .then((response) => {
          ...
          // Check available assets to confirm API is working
          return derivAPI.getTradableAssets()
        })
        .then((assets) => {
          console.log("Available trading assets:", assets)
          // Automatically fetch initial history once connected
          return fetchInitialHistory()
        })
        .catch((error) => {
    ...
    derivAPI.onClose(() => {
      ...
      // Generate offline data if we're not in initialization phase
      if (!isInitializing) {
        generateOfflineData()
      }
    })
    ...
    // Add a periodic check to ensure we're connected
    ...
    return () => {
      // Cleanup - will properly disconnect on component unmount
  ...
  // Generate offline data when API connection fails
  const generateOfflineData = () => {
    ...
    // Update digit percentages based on offline data
    ...
    // Start a offline tick simulation
    startOfflineTickSimulation()
  }

  // Simulate ticks for offline mode
  ...
  const fetchInitialHistory = async () => {
    try {
      console.log("Fetching history for symbol:", volatilitySymbol)
  ...
  return (
    ...
    >
      ...
      >
        <Button variant="ghost" size="icon" className={isDarkTheme ? "text-white" : "text-gray-900"}>
